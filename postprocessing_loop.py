    ...
    for epoch in range(150 if args.train else 1):
        ...
        if args.train:
            ...
            if(epoch<=46):
                ...
                for sample in iterator:
                    ...
                    if(epoch==46):
                        ...
                        for i in range(rgb.shape[0]):
                            tiled_rgb_data = [rgb[i,:,x:x+tile_h,y:y+tile_w] for x in range(0,rgb.size(2),tile_h) for y in range(0,rgb.size(3),tile_w)]
                            tiled_rgb_data_list.append(tiled_rgb_data)
                            tiled_output_data = [outputs[i,:,x:x+tile_h,y:y+tile_w] for x in range(0,outputs.size(2),tile_h) for y in range(0,outputs.size(3),tile_w)]
                            tiled_output_data_list.append(tiled_output_data)
                            tiled_label_data = [target[i,:,x:x+tile_h,y:y+tile_w] for x in range(0,outputs.size(2),tile_h) for y in range(0,outputs.size(3),tile_w)]
                        ...
                        cutmix_patch = []
                        cutmix_label_patch = []
                        # print(len(tiled_output_data_list))  
                        for i in range(len(tiled_output_data_list)):
                            ...
                            for j in range(len(tiled_output_data_list[i])):
                                ...
                                #print(len(tiled_losses))
                            
                            #v_min, v_max = tiled_output_data[i].min(), tiled_output_data[i].max()
                            #new_min, new_max = 0.01, 1.0
                            #print(tiled_output_data[i].shape)
                            #v_p = (tiled_output_data[i] - v_min)/(v_max - v_min)*(new_max - new_min) + new_min
                            #dived = torch.div(tiled_label_data[i],v_p)
                            
                           
                            #tiled_losses.append(criterion(dived, tiled_label_data[i]))
                            
                            #tiled_losses.append(torch.sum(torch.div(tiled_label_data[i],v_p),dim=0))
                            #print(torch.div(tiled_label_data[i],v_p),dim=1)
                        ...
                        #print(len(cutmix_patch))
                        #print(cutmix_patch[0])
                        ...
                        for k in range(len(cutmix_patch)):
                            ...
                            skimage.io.imsave(os.path.join('/home/user/Perception/SN6_dataset/train_set/AOI_11_Rotterdam/cu_RGB/', (sample['img_name'][k][74:-4])+'_cutmix_aug_46.tif'), rgb_full)
                            target_full_name = os.path.join('/home/user/Perception/SN6_dataset/train_set/AOI_11_Rotterdam/cu_mas', os.path.basename(sample["img_name"][k]).replace('PS-RGB', 'SAR-Intensity'))
                            skimage.io.imsave(os.path.join('/home/user/Perception/SN6_dataset/train_set/AOI_11_Rotterdam/cu_mas/', (target_full_name[74:-4])+'_cutmix_aug_46.tif'), target_full)
                ...
                scheduler.step()
                #print("Current_LR:", scheduler.get_lr()[-1])
                torch.save({'epoch': epoch, 'state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'loss': loss}, path.join(models_folder, args.snapshot_last))
                torch.cuda.empty_cache()
            else:
                
                train_rgb_img_files_for_cutmix = sorted([f for f in glob.glob(os.path.join('/home/user/Perception/SN6_dataset/train_set/AOI_11_Rotterdam/cu_RGB/*.tif'))])
                train_label_files_for_cutmix = sorted([f for f in glob.glob(os.path.join('/home/user/Perception/SN6_dataset/train_set/AOI_11_Rotterdam/cu_mas/*.tif'))])
                train_label_index_files_for_cutmix = sorted([f for f in glob.glob(os.path.join('/home/user/Perception/SN6_dataset/train_set/AOI_11_Rotterdam/label_index_masks_for_cutmix/*.tif'))])
                ...
                for sample in iterator_2:
                    ...
                    for k in range(len(cutmix_patch)):
                        ...
                        if('_cutmix_aug_' not in sample["img_name"][k]):
                            ...
                            skimage.io.imsave(os.path.join('/home/user/Perception/SN6_dataset/train_set/AOI_11_Rotterdam/cu_RGB/', (sample['img_name'][k][74:-4])+'_cutmix_aug_'+str(epoch)+'.tif'), rgb_full)
                            target_full_name = os.path.join('/home/user/Perception/SN6_dataset/train_set/AOI_11_Rotterdam/cu_mas', os.path.basename(sample["img_name"][k]).replace('PS-RGB', 'SAR-Intensity'))
                            skimage.io.imsave(os.path.join('/home/user/Perception/SN6_dataset/train_set/AOI_11_Rotterdam/cu_mas/', (target_full_name[74:-4])+'_cutmix_aug_'+str(epoch)+'.tif'), target_full)
                        else:
                            pass
                    l0 = args.focal_weight * focal_loss(outputs[:, 0], target[:, 0], weights[:, 0]) + dice_loss(outputs[:, 0], target[:, 0])
                    l1 = args.edge_weight * (args.focal_weight * focal_loss(outputs[:, 1], target[:, 1], weights[:, 1]) + dice_loss(outputs[:, 1], target[:, 1]))
                ...
                for f in glob.glob("/home/user/Perception/SN6_dataset/train_set/AOI_11_Rotterdam/cu_RGB/*_cutmix_aug_"+str(epoch-1)+".tif"):
                    os.remove(f)
                for f in glob.glob("/home/user/Perception/SN6_dataset/train_set/AOI_11_Rotterdam/cu_mas/*_cutmix_aug_"+str(epoch-1)+".tif"):
                    os.remove(f)
        ...
        if (args.val or epoch >= args.start_val_epoch) or args.test:
            ...
            model.eval()
            #target_layers = [model.encoder._blocks[25]]
            torch.cuda.empty_cache()
            with torch.no_grad():
                for _, sample in enumerate(tqdm(val_data_loader)):
                    
                    rgb = sample["rgb_full"].cuda(non_blocking=True)
                    ...
                    for sc in scales:
                        ...
                        rgb = F.interpolate(rgb, size=(512,512), mode = 'bilinear', align_corners=True)
            ...
            for seedth in seedth_list:
                for predth in predth_list:
                    for areath in areath_list:
                        if (args.val or epoch >= args.start_val_epoch) or args.test:
                            ...
                            print("areath and predth", seedth,predth)
                            test_postprocess(args.pred_folder, args.pred_csv, seed_msk_th = seedth, pred_th = predth, areath=areath )  
                            
                            val_f1,tp,fp,fn = evaluation(args.pred_csv,args.masks_csv,areath)

                            print("AREA TH for this run: ", areath)
                            
                            print()
                            print('    Validation F1 score at epoch {}: {:.5f}, best {}'.format(epoch, val_f1, max(val_f1, best_f1)))
                            print()
                            print(val_f1,best_f1)
                            print(best_f1<val_f1)
                            if best_f1 < val_f1:
                                best_f1 = val_f1
                                best_seedth = seedth
                                best_predth = predth
                                best_areath = areath
                                print("New best valf1 with seedth, predth and areath: ", best_seedth, best_predth, best_areath )
                                #torch.save({'epoch': epoch, 'state_dict': to_save, 'best_score': val_f1}, path.join(models_folder, args.snapshot_best))
                            #q.put(val_f1)
                            print("Overall BEST valf1 with seedth and predth ", best_f1, best_seedth, best_predth, best_areath)
                            print()
                            #items = [areath,val_f1,tp,fp,fn]
                            #towrite.append(items)
                            #file = open('wdata_Baseline_EO_withcutmix_100.txt','w')
                            #for item in towrite:
                            #    file.write(str(item)+"\n")
                            #file.close()
                            tp=0
